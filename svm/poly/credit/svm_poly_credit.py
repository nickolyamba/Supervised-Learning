...
if __name__ == "__main__":
    ...
    # Cross-Validation
    cv = cross_validation.StratifiedShuffleSplit(train_Y, n_iter=1,test_size=0.2, random_state=42)
    examples = np.array([10, 50, 100, 500, 1000, 5000, 16800])
    gamma = 0.5
    degree = 3
    ...
    heldout_score = heldout_score*100
    
    title = "Learning Curves (SVM, Poly kernel, $\gamma={0}$, degree={1})\n" \
    "held out score={2:.2f}%, train time={3:.2f}".format(gamma, degree, heldout_score, train_time)
    
    estimator = SVC(kernel='poly',gamma=gamma, degree=degree, cache_size=500)
    plot_learning_curve(estimator, title, train_X, train_Y, (0.5, 1.0), cv=cv, n_jobs=-1, 
